...
<div class="grid cols-3" style="margin-bottom:20px">
  <div class="card stat">
    ...
    <div class="val tnum">{{ number_format($craftsmen->count()) }}</div>
  </div>
  <div class="card stat">
    ...
    <div class="val tnum" style="color:var(--pos)">{{ number_format($totalPaid) }} <small>ج.م</small></div>
  </div>
  <div class="card stat">
    ...
    <div class="val tnum" style="color:{{ $totalRemaining > 0 ? 'var(--neg)' : 'var(--pos)' }}">{{ number_format($totalRemaining) }} <small>ج.م</small></div>
  </div>
</div>
  ...
  <div class="table-card" style="margin-bottom:16px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;padding:14px 18px;border-bottom:1px solid var(--line)">
      ...
      <div style="display:flex;gap:20px;text-align:center">
        <div><div class="muted" style="font-size:12px">متعاقد</div><div class="tnum" style="font-weight:700">{{ number_format($c->contracted) }}</div></div>
        <div><div class="muted" style="font-size:12px">مدفوع</div><div class="tnum" style="font-weight:700;color:var(--pos)">{{ number_format($c->paid) }}</div></div>
        <div><div class="muted" style="font-size:12px">متبقي</div><div class="tnum" style="font-weight:700;color:{{ $c->remaining > 0 ? 'var(--neg)' : 'var(--pos)' }}">{{ number_format($c->remaining) }}</div></div>
      </div>
    </div>
            ...
            <tr>
              ...
              <td class="muted">
                ...
                @if(in_array($a->contract_type, ['per_meter','per_piece','daily']) && $a->contract_qty)
                  <span style="font-size:12px">({{ rtrim(rtrim(number_format($a->contract_qty, 2), '0'), '.') }} × {{ number_format($a->contract_unit_rate) }})</span>
                @endif
              </td>
              <td class="num">{{ number_format($a->amount) }}</td>
              <td class="num" style="color:var(--pos)">{{ number_format($paid) }}</td>
              <td class="num" style="color:{{ $remaining > 0 ? 'var(--neg)' : 'var(--pos)' }}">{{ number_format($remaining) }}</td>
