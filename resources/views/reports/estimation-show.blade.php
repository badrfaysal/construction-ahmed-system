...
<div class="grid cols-5" style="margin-bottom:14px">
  ...
  <div class="vstat vstat-red">
    <div class="top"><span class="label">بنود فرعية</span>
...
<div class="card" style="padding:14px 18px;margin-bottom:20px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;font-size:13px">
  ...
  <span class="tag red">بنود فرعية {{ number_format($area > 0 ? $totalPettyCost / $area : 0, 1) }} ج.م</span>
  <span style="color:var(--ink-3)">+</span>
  ...
  <div class="table-card est-band"
    ...
    <div class="est-band-head">
      ...
      <div class="est-band-costs">
        ...
        <div class="ebc"><span class="l">بنود فرعية</span><span class="v" style="color:var(--neg)">{{ \App\Support\Money::format($row->petty_cost) }}</span></div>
    ...
    @if($row->petty->count())
      <div class="est-sec-lbl" style="color:var(--neg);background:color-mix(in srgb, var(--neg) 4%, transparent)">
        ...
        بنود فرعية ({{ $row->petty->count() }} مصروف)
      </div>
      <div class="table-scroll">
        <table>
          ...
          <tfoot><tr><td colspan="2" style="color:var(--neg)">إجمالي البنود الفرعية</td><td class="num" style="color:var(--neg)">{{ \App\Support\Money::format($row->petty_cost) }}</td></tr></tfoot>
        </table>
      </div>
    @endif
